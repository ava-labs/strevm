...
func (s *Stub) BuildHeader(parent *types.Header) *types.Header {
	...
	canonicalExtra := extra{
		nanoseconds: uint64(now.Nanosecond()), //nolint:gosec // Known non-negative
	}
	hdr := &types.Header{
...
func BuildBlock(
	...
	// in the built block.
	if err := canonicalExtra.UnmarshalCanoto(header.Extra); err != nil {
		return nil, err
	}

	canonicalExtra.ops = make([]op, len(ops))
	for i, hookOp := range ops {
		canonicalExtra.ops[i] = opFromHookOp(hookOp)
	}

	header.Extra = canonicalExtra.MarshalCanoto()
	...
	return &Stub{
		Now: func() time.Time {
			return time.Unix(
				...
				int64(canonicalExtra.nanoseconds), //nolint:gosec // Won't overflow
			)
		},
		Ops: ops,
	}
...
func (s *Stub) SubSecondBlockTime(hdr *types.Header) time.Duration {
	...
	return time.Duration(canonicalExtra.nanoseconds) //nolint:gosec // Test-only code that relies on our own encoding of nanonseconds in [Stub.BuildHeader]
}
...
type extra struct {
	nanoseconds uint64 `canoto:"uint,1"`
