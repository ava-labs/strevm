...
func (vm *VM) newBlock(eth *types.Block, parent, lastSettled *blocks.Block) (*blocks.Block, error) {
	return blocks.New(eth, parent, lastSettled, vm.hooks.GasTarget, vm.log())
}
...
func (vm *VM) buildBlock(ctx context.Context, parent *blocks.Block, blockTime time.Time, candidates []*txgossip.LazyTransaction) (*blocks.Block, error) {
	...
	for _, b := range history {
		// if err := hook.BeforeBuildingBlock(vm.hooks, b.ParentBlock().EthBlock()); err != nil {
		// 	return nil, fmt.Errorf("hook.BeforeBlock(): %v", err)
		// }
		hook.AfterBuildingBlock(clock, b.Transactions())
	}

	blockNum := new(big.Int).Add(parent.Number(), big.NewInt(1))
	blockTimeStamp := unix(blockTime)

	// if err := hook.BeforeBuildingBlock(); err != nil {
	// 	return nil, err
	// }
