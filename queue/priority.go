...
func (p *Priority[T]) Pop() T {
	return heap.Pop(&p.p).(T) //nolint:forcetypeassert // Invariant maintained by [Priority.Push].
}
...
func (p *priority[T]) Push(x any) {
	p.append(x.(T)) //nolint:forcetypeassert // Invariant maintained by [Priority.Push].
}
